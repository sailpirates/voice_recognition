...
class JSONStringTextSource : public JSONTextSource
  ...
  public:
    JSONStringTextSource(const char *text) : text(text), current(text)
      { assert(text != NULL); }
...
class JSONParser
  ...
  private:
    ...
    void do_exponent(o_integer mantissa_whole_part,
            o_integer mantissa_fraction_numerator,
            size_t mantissa_fraction_digit_count, JSONHandler *handler);
    o_integer parse_decimal_digits(size_t *digit_count_location = NULL);
    int getchar(void)
      {
    ...
    void error(const char *format, ...)
      {
        ...
        if (file_name == NULL)
            file_name = "???";
        fprintf(stderr, "Error [%s:%lu:%lu]: ", file_name,
            ...
            switch (next)
              {
                case '.':
                  {
                    ...
                    o_integer fraction =
                            parse_decimal_digits(&fraction_digit_count);
                    ...
                    if ((trailer == 'e') || (trailer == 'E'))
                      {
                        do_exponent(the_oi, fraction, fraction_digit_count,
                                    handler);
                        return;
                      }
                    if (trailer != EOF)
                        ungetchar(trailer);
                    handler->number_value(the_oi, fraction,
                                          fraction_digit_count, oi_zero);
                    oi_remove_reference(fraction);
                    oi_remove_reference(the_oi);
                    return;
                  }
                ...
                case 'E':
                  {
                    do_exponent(the_oi, oi_zero, 0, handler);
                    return;
                  }
                default:
                  {
...
void JSONParser::do_exponent(o_integer mantissa_whole_part,
        o_integer mantissa_fraction_numerator,
        size_t mantissa_fraction_digit_count, JSONHandler *handler)
  {
    int inchar = getchar();
    ...
    o_integer exponent = parse_decimal_digits();
...
o_integer JSONParser::parse_decimal_digits(size_t *digit_count_location)
  {
    int inchar = getchar();
    assert((inchar >= '0') && (inchar <= '9'));
...
extern void parse_json_value(const char *text, JSONHandler *handler)
  {
    JSONStringTextSource source(text);
    parse_json_value(&source, handler);
  }

extern void parse_json_value(std::string text, JSONHandler *handler)
  {
    parse_json_value(text.c_str(), handler);
  }
