...
class ConversationStateJSON : public ReferenceCounted
  ...
  private:
    std::string contents;
    ConversationStateJSON(const ConversationStateJSON &)  { assert(false); }
    ConversationStateJSON &operator=(const ConversationStateJSON &other)  { assert(false); throw "Illegal operator=() call."; }
  ...
  public:
    ...
    void write_as_json(JSONHandler *handler) const
      {
        parse_json_value(contents, handler);
      }
